...
import DataTable from '../../Components/TeamsCompo/TeamsCompo';
import AddItemForm from '../../Components/TeamsCompo/AddItmeCompo';
import EditItemForm from '../../Components/TeamsCompo/EditeFormCompo'; // Import EditItemForm
...
function TeamsAdmin() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get('http://localhost:3000/teams/getData');
  ...
  const handleDelete = (item) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`http://localhost:3000/teams/delete/${item._id}`);
